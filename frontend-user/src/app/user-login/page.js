...
const Page = () => {
  ...
  const onSubmitRegister = async (data) => {
    ...
    try {
      const payload = {
        loginType: type, // Thêm key để backend biết đây là email hay phone
        [type]: value,   // Gửi giá trị tương ứng
        username: data.tenND,
        password: data.matKhau,
        birthdate: data.ngaySinh.toISOString().split("T")[0],
      ...
      if (response.ok) {
        ...
        toast.error(result.message || `Đăng ký thất bại (Mã lỗi: ${response.status})`, {
          style: { background: "#fee2e2", color: "#b91c1c" },
        });
      }
    } catch (error) {
      console.error("Lỗi khi đăng ký:", error);
      toast.error(error.message || "Có lỗi xảy ra khi kết nối với server", {
        style: { background: "#fee2e2", color: "#b91c1c" },
      });
    }
  };
  ...
  const onSubmitOTP = async (data) => {
    try {
      ...
      if (verifyResponse.ok) {
        ...
        toast.error(verifyResult.message || `Xác thực OTP thất bại (Mã lỗi: ${verifyResponse.status})`, {
          style: { background: "#fee2e2", color: "#b91c1c" },
        });
      }
    } catch (error) {
  ...
  const onSubmitLogin = async (data) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(result.message || `Đăng nhập thất bại (Mã lỗi: ${response.status})`, {
          style: { background: "#fee2e2", color: "#b91c1c" },
        });
